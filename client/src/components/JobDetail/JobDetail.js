...
class JobDetail extends Component {
	...
	removeMaterials = (id) => {
		const currentJob = this.state.jobNumber;
		console.log("this.state.jobNumber: ", currentJob);
		console.log("this.state.material: ", this.state.materialList);
		for (let i = 0; i < this.state.materialList.length; i++) {
			// console.log("If we're getting here, here's i: ", i);
			if (!(this.state.materialList[i].materialQuantity - this.state.materialQuantity >= 0) && (this.state.material === this.state.materialList[i].name)) {
				alert("Not enough materials. You need to order more!!!");
			} else if ((this.state.materialList[i].materialQuantity - this.state.materialQuantity >= 0) && (this.state.material === this.state.materialList[i].name)) {
				const inputNum = this.state.materialList[i].materialQuantity;
			...
			// else {
				// alert("Not enough materials. You need to order more!!!");
				// return null;
				// TODO: Need to not do the alert, but below stuff isn't working
				// (
	...
	render() {
		// TODO: Need to make an error box if there aren't enough materials instead of our ugly alert
		// if (this.state.tooMany) {
		// 	return (
		// 		<React.Fragment>
		// 			<h1>You don't have enough materials!!!</h1>
		// 		</React.Fragment>
		// 	)
		// }
